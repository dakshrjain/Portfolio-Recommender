...
df = pd.read_csv(datadir_final+"Large_cap_equities/500800.csv")
...
# Combine SARIMA and Random Forest forecasts
forecast_period = 21  # activew trading days in a month
final_forecasts = {}
...
for feature in features:
    ...
    # Forecast residuals using the Random forest model
    future_steps = np.arange(len(data) + 1, len(data) + forecast_period + 1).reshape(-1, 1)
    ml_forecast = rf[feature].predict(future_steps)
